...
RHFResult rhf_closed_shell(const libint2::BasisSet& basis,
    ...
    double E_old = 0.0;
    dft::DIISManager diis(diis_max_vec);
    RHFResult result;
    ...
    for (int iter = 1; iter <= max_iter; ++iter) {
        ...
        const Eigen::MatrixXd err = dft::DIISManager::error_matrix(F_raw, D, S, X);
...
double rhf_total_energy_quiet(const Molecule& mol,
    ...
    RHFResult rhf = rhf_closed_shell(basis, S, Hcore, nelec, mol,
                                     max_iter,
                                     1.0e-10,
                                     1.0e-8,
                                     true,
...
Eigen::VectorXd rhf_analytic_gradient_quiet(const Molecule& mol,
                                            const std::string& basis_name,
                                            int max_iter = 128,
                                            const Eigen::MatrixXd* C_guess = nullptr,
                                            Eigen::MatrixXd* C_out = nullptr) {
    ...
    const int nelec = electron_count(mol);
    if (nelec <= 0) {
        throw std::runtime_error("non-positive electron count");
    }
    if (nelec % 2 != 0) {
        throw std::runtime_error("closed-shell RHF needs an even electron count");
    }
    ...
    const Eigen::MatrixXd S =
        compute_1body_matrix(libint2::Operator::overlap, basis, mol.atoms);

    const Eigen::MatrixXd T =
        compute_1body_matrix(libint2::Operator::kinetic, basis, mol.atoms);

    const Eigen::MatrixXd V =
        compute_1body_matrix(libint2::Operator::nuclear, basis, mol.atoms);

    const Eigen::MatrixXd Hcore = T + V;
    ...
    const Eigen::MatrixXd grad_mat =
        miniqc::rhf_analytic_gradient(mol.atoms,
                                      basis,
                                      rhf.D,
                                      rhf.C,
                                      rhf.eps,
    ...
    return miniqc::flatten_atom_gradient_rowwise(grad_mat);
}
    ...
    try {
        ...
        for (int i = 6; i < argc; ++i) {
            ...
            if (arg == "--print-matrices") {
                ...
                do_lda_x = true;
                if (i + 2 >= argc) throw std::runtime_error("--lda-x requires n_radical and angular_grid");
                std::string a = argv[i+1];
                n_radial = std::stoi(a);
                std::string b = argv[i+2];
                angular_grid = std::stoi(b);
                i=i+2;
            } else {
        ...
        if (do_optimization && (do_mp2 || do_fci || do_lda_x)) {
            throw std::runtime_error("--mp2/--fci/--CISD/--CISD/--lda-x are currently implemented for single-point calculations only; run optimization first, then run MP2/FCI on the optimized XYZ file");
        }
        ...
        if (do_optimization) {
            ...
            GradientFunction gradient = [&](const Eigen::VectorXd& x,
                                            const Eigen::MatrixXd* C_guess,
                                            Eigen::MatrixXd* C_out) -> Eigen::VectorXd {
                const Molecule trial = molecule_with_coordinate_vector(mol0, x);
                return rhf_analytic_gradient_quiet(trial,
                                                basis_name,
                                                128,
                                                C_guess,
                                                C_out);            
            ...
            GeometryResult opt = optimize_bfgs(x0, energy, geom_options, gradient);
            ...
            write_xyz_angstrom(opt_output_xyz, opt_mol,
                                "optimized by miniqc RHF analytic-gradient BFGS; coordinates in Angstrom");
        ...
        RHFResult rhf = rhf_closed_shell(basis, S, Hcore, nelec, mol);
        miniqc::check_overlap_derivative_finite_difference(
            basis_name,
            mol.atoms,
            0,       // atom index
            0,       // x direction
            true,    // force_cartesian; match basis.set_pure(false)
            1.0e-4,
            true
        );

        miniqc::check_kinetic_derivative_finite_difference(
            basis_name,
            mol.atoms,
            0,
            0,
        ...
        miniqc::check_nuclear_attraction_derivative_finite_difference(
            basis_name,
            mol.atoms,
            0,
            0,
        ...
        miniqc::check_rhf_twoelectron_derivative_finite_difference(
            basis_name,
            mol.atoms,
            rhf.D,
            0,
        ...
        const Eigen::MatrixXd grad_rhf =
            miniqc::rhf_analytic_gradient(
                mol.atoms,
                basis,
                rhf.D,
                rhf.C,
        ...
        std::cout << "RHF analytic gradient, dE/dR, Hartree/bohr:\n"
              << std::scientific << std::setprecision(12)
              << grad_rhf << "\n";

        std::cout << "RHF force, -dE/dR, Hartree/bohr:\n"
              << -grad_rhf << "\n";

        std::cout << "gradient max abs = "
              << grad_rhf.cwiseAbs().maxCoeff() << "\n";
        ...
        if (do_lda_x) {
            ...
            // First recommended atom-centered grid.
            // Coordinates are already in bohr because read_xyz_atoms() converted them.
            grid_options.n_radial = n_radial;
            grid_options.angular_grid = angular_grid;       // supported: 6, 14, 26
            grid_options.r_max = 12.0;            // bohr
            grid_options.radial_type = dft::RadialGridType::PowerMapped;
            grid_options.radial_power = 2.0;      // r = r_max * t^2
            grid_options.partition_type = dft::PartitionType::Becke;
            grid_options.becke_smooth_order = 3;

            // Simple distance partition:
            //   q_A = 1 / (|r - R_A| + eps)^p
            //   P_A = q_A / sum_B q_B
            grid_options.partition_eps = 1.0e-10;
            grid_options.partition_power = 4.0;
            grid_options.weight_cutoff = 0.0;
            grid_options.verbose = true;

            auto grid = dft::make_atom_centered_grid(mol.atoms, grid_options);

            dft::RKSLDAOptions lda_options;
            lda_options.max_iter = 256;
            lda_options.e_conv = 1.0e-10;
            lda_options.d_conv = 1.0e-8;
            lda_options.density_mixing = 0.3;
            lda_options.verbose = true;
            lda_options.functional = dft::LDAFunctional::Libxc_GGA_PBE;

            auto lda = dft::run_rks_lda_exchange_only_sp(
                basis, S, Hcore, nelec, Enuc, grid, lda_options
            );

            std::cout << "\n=== RKS LDA exchange-only result ===\n";
            std::cout << std::fixed << std::setprecision(12);
            std::cout << "converged  = " << lda.converged << "\n";
            std::cout << "niter      = " << lda.niter << "\n";
            std::cout << "E_x        = " << lda.E_x << " Ha\n";
            std::cout << "E_c        = " << lda.E_c << " Ha\n";
            std::cout << "Ne(grid)   = " << lda.Ne_grid << "\n";
            std::cout << "E_total    = " << lda.E_total << " Ha\n";
        }
        if (print_matrices) {
            std::cout << "density D:\n" << rhf.D << "\n\n";
            std::cout << "MO C:\n" << rhf.C << "\n";
